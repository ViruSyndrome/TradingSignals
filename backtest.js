...
async function runParameterSweep(histories, fgMap) {
  console.log('\n═══════════════════════════════════════════════════════════════');
  ...
  let totalCombs = holdLimits.length * emaFastList.length * emaSlowList.length * rsiPeriods.length;
  let currentComb = 0;

  process.stdout.write(`  Testing ${totalCombs} combinations... `);

  for (const hLimit of holdLimits) {
    for (const eFast of emaFastList) {
      for (const eSlow of emaSlowList) {
        for (const rPeriod of rsiPeriods) {
          currentComb++;
          if (currentComb % 10 === 0) process.stdout.write('.');
          ...
          // Quick evaluate on top volume assets to save time, or all if preferred.
          // Using the first 20 assets for speed of sweep
          const sweepAssets = CONFIG.assets.crypto.slice(0, 20);
          ...
          for (const asset of sweepAssets) {
            ...
            const stats = computeStats(trades, 'returnPct');
            if (stats) {
              // Score based on average return * winRate penalty, etc. Simple net return is fine.
              totalNetReturn += parseFloat(stats.avgReturn) * stats.totalTrades;
              totalTradesCount += stats.totalTrades;
            }
          }

          const avgNet = totalTradesCount > 0 ? (totalNetReturn / totalTradesCount) : -Infinity;
          
          if (avgNet > bestScore) {
            bestScore = avgNet;
            bestParams = { ...CONFIG.activeParams };
          }
        }
      }
    }
